...
func VerificarCaptcha(response string) (bool, error) {
	if response == "" {
		return false, nil
	}
...
func RegisterHandler(c *fiber.Ctx) error {
	...
	captchaOk, captchaErr := VerificarCaptcha(captchaResponse)
	if captchaErr != nil {
		return c.Render("register", fiber.Map{
			"Error": "No se pudo validar el captcha. Intenta nuevamente más tarde.",
			"Datos": nil,
		})
	}
	if !captchaOk {
	...
	type input struct {
		...
		Pase2    string `json:"pase2" form:"pase2"`
	...
	if err := c.BodyParser(&in); err != nil {
		return c.Status(fiber.StatusBadRequest).Render("register", fiber.Map{
			"Error": "Formulario inválido. Por favor vuelve a intentarlo.",
			"Datos": nil,
		})
	}
	...
	if in.Nombre == "" {
		return c.Render("register", fiber.Map{"Error": "El nombre es obligatorio.", "Datos": in})
	}
	if in.Apellido == "" {
		return c.Render("register", fiber.Map{"Error": "El apellido es obligatorio.", "Datos": in})
	}
	...
	if in.Pase != in.Pase2 {
		return c.Render("register", fiber.Map{"Error": "Las contraseñas no coinciden", "Datos": in})
	}
	...
	if err := db.DB.Create(&user).Error; err != nil {
		return c.Render("register", fiber.Map{"Error": "Error al guardar en BD", "Datos": in})
	}
