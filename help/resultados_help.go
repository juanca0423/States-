...
func RecCol1(Balance map[string]float64, Recorido []models.Cue, titulo string, vTotalVariables, vTotalFijos, vGtosNoEfectivo *float64) ([]models.KR, float64) {
	return recColGrupo(Balance, Recorido, titulo, 0, nil, vTotalVariables, vTotalFijos, vGtosNoEfectivo)
}

// RecCol1Tot with cumulative sum column
func RecCol1Tot(Balance map[string]float64, Recorido []models.Cue, titulo string, SumaAnterior float64, vTotalVariables, vTotalFijos, vGtosNoEfectivo *float64) ([]models.KR, float64) {
	callback := func(anterior, grupo float64) (float64, bool) {
		return anterior + grupo, true
	}
	return recColGrupo(Balance, Recorido, titulo, SumaAnterior, callback, vTotalVariables, vTotalFijos, vGtosNoEfectivo)
}

// RecCol2Tot with cumulative difference column
...
// recColGrupo is the shared implementation for RecCol* functions.
// It returns the resulting slice and the group subtotal.
func recColGrupo(Balance map[string]float64, Recorido []models.Cue, titulo string, sumaAnterior float64, col3Calc func(anterior, grupo float64) (float64, bool), vTotalVariables, vTotalFijos, vGtosNoEfectivo *float64) ([]models.KR, float64) {
	var res []models.KR
	var SaldoGrupo float64 // subtotal for this group
	...
	for _, v := range Recorido {
		...
		SaldoGrupo += val
		res = append(res, models.KR{Key: strconv.Itoa(v.Codigo), Value: models.ReString{Nombre: v.Nombre, Col1: config.FCont(val)}})
	}
	...
	finalRes := []models.KR{{Key: "TIT-" + titulo, Value: models.ReString{Nombre: titulo, ClasNombre: "fw-bold"}}}
	finalRes = append(finalRes, res...)

	// Always set column 2 with group total
	finalRes[len(finalRes)-1].Value.Col2 = config.FCont(SaldoGrupo)

	// If a col3Calc is provided, compute column 3 and set its style
	if col3Calc != nil {
		if val, ok := col3Calc(sumaAnterior, SaldoGrupo); ok {
			finalRes[len(finalRes)-1].Value.Col3 = config.FCont(val)
